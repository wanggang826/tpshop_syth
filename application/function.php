 ...
 * $desc  支付描述
 */
function creditLog($user_id,$order_id,$pay_credit,$desc){
    $data = [
        'user_id'=>$user_id,
        'order_id'=>$order_id,
...
/**
 * 支付成功修改分期状态(通过$order_id)
 * @param $str
 * @return string
 */
function  staging_edit($order_id){
  $list =  M('Staging')->where(['order_id'=>$order_id,'status'=>2])->field(['id','status'])->select();
  if($list){
      foreach ($list as $k=>$v){
          M('Staging')->where(['id'=>$v['id']])->update(['status'=>0]);
      }
  }
}
...
/**
 * 获取url 中的各个参数  类似于把 pay_code=alipay&bank_code=ICBC-DEBIT  处理成数组 array('pay_code'=>'alipay')
 * @param type $str
 * @return type
 */
...
function logger($content){
    M('pay_log')->add(array('content'=>$content, 'create_time'=>time()));
}

/**
 * 邀请人以及邀请人的上级级别
 * $user_id   邀请人用户ID
 * requery arr()  返回邀请人以及邀请人的所有上级的职位以及用户ID
 */
function invite($user_id){
    $member = M('Users')->where(['user_id'=>$user_id])->field(['position','client','business','area'])->find();
    if($member['position'] ==1){
        return ['area'=>$user_id];
    }elseif($member['position'] ==2){
        return ['business'=>$user_id,'area'=>$member['area']];
    }elseif($member['position'] ==3){
        return ['client'=>$user_id,'business'=>$member['business'],'area'=>$member['area']];
    }else{
        return false;
    }
}
 ...
 * 二维码(商品)
 */
function make_qrcode($url, $path){
    $value = $url;                  //二维码内容
    $errorCorrectionLevel = 'H';    //容错级别
    $matrixPointSize = 5;           //生成图片大小
    //生成二维码图片
    QRcode::png($value, $path , $errorCorrectionLevel, $matrixPointSize, 2);
    $logo ="./public/upload/logo/biaozhi/biaozhi.png";//准备好的logo图片
    $QR  = $path;//已经生成的原始二维码图
    if($logo !== FALSE) {
    ...
    imagepng($QR, $path);
    imagedestroy($QR);
    $imgUrl = Request::instance()->domain().ltrim($path,'.');
    return $imgUrl;
}
 ...
 * 二维码生成
 */
function createInviteQrcode($url_data,$user_id){
...
function make_order_sn()
{
    ...
    // 保证不会有重复订单号存在
    while(true){
        $order_sn = rand_code().'recharge'; // 充值订单编号
        $order_sn_count = M('recharge')->where(array("order_sn"=>$order_sn))->count();
        if($order_sn_count == 0)
            break;
    }
