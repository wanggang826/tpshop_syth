...
class User extends Base {
    ...
    public function ajaxindex(){
       ...
       /* I('first_leader') && ($condition['first_leader'] = I('first_leader')); // 查看一级下线人有哪些
        ...
        $userList = $model->where($condition)->order($sort_order)->select();//limit($Page->firstRow.','.$Page->listRows)->
        foreach ($userList as $k => &$v){
            $v['leaver_num']  = count(junior($v['user_id']));
            if($v['leaver_num']){
                $junior_money=junior_money($v['user_id']);
                if(!empty($junior_money['points']) || !empty($junior_money['money'])){
                    if(!empty($junior_money['points'])){
                        $points = array_sum(array_map(create_function('$val', 'return $val["pay_points"];'), $junior_money['points']));//积分消费总额
                    }else{
                        $points= floatval(0);
                    }
                    if(!empty($junior_money['money'])){
                        $money = array_sum(array_map(create_function('$val', 'return $val["user_money"];'), $junior_money['money']));//现金消费总额
                    }else{
                        $money=  sprintf('%.2f', 0);
                    }
                    $v['leaver_money'] = abs($points + $money);
                }else{
                    $v['leaver_money'] =sprintf('%.2f', 0);
                }
            }else{
                $v['leaver_money'] =sprintf('%.2f', 0);
            }
    ...
    public function points_list(){
        ...
        $data = $logic->get_account_log($user_id, 1);
    ...
    public function xinyong_list(){
        ...
        $list = model('Credit')->getCredit(['user_id' => $user_id, 'page' => $page]);//where(['user_id'=>$user['user_id']])->select();
        $page = $list->render();
    ...
    public function money_list(){
        ...
        $logic = new UsersLogic();
        $logic = new UsersLogic();
        $data = $logic->get_account_log($user_id, 2);
        $account_log = $data['result'];
        $this->assign('account_log', $account_log);
        $this->assign('page', $data['show']);
        return $this->fetch();
     ...
     * 会员下级列表
     */
    public function laver_list(){
        $id                    = input('user_id');//上级ID
        $superior              = ".".$id.".";//拼接上级ID
        $where['leaders']      = array('like', "%$superior%");
        $list                  = model('Users')->where($where)->order('user_id')->field('user_id, position, level, reg_time, last_login,nickname,mobile')->select();
        if (!empty($list)) {
            $grade             = ['1'=>'普通会员', '5'=>'VIP会员'];//会员级别
            $duty              = ['无职务', '区域经理', '业务经理', '客户经理'];//会员职务
            foreach ($list as $k => $v) {
                $v['position'] = $duty[$v['position']];
                $v['level']    =   $grade[$v['level']];
            }
        }
        $this->assign('lists',$list);
        return $this->fetch();
    ...
    public function detail(){
        ...
        $managerList    = model('Manager')->select();//所有职称
        if(!$user)
        ...
        if(IS_POST){
            ...
            if(!empty($data['pay_points'])){
                $reg = '/^((?:-?0)|(?:-?[1-9]\d*))(?:\.\d{1,2})?$/';
                if(!preg_match($reg,$data['pay_points'])){
                    exit($this->error('积分格式有误!'));
                }
            }
            if(!empty($data['email']))
            ...
            //如果推荐人发生变化
            if(isset($data['first_leader']) && !empty($data['first_leader'])){
                $new = M('users')->where(['user_id' => $data['first_leader']])->field('position,leaders,client,business,area')->find();
                if(!$new)  return $this->error('推荐人不存在!');
                if(!empty($new['leaders'])){
                    $new['leaders'] = $new['leaders'].$data['first_leader'].".";//推荐人
                }else{
                    $new['leaders']=".".$data['first_leader'].".";//推荐人
                }
                $new['first_leader'] =$data['first_leader'];
            }
            $job = ['1'=>'area','2'=>'business','3'=>'client'];//职称
            if($data['position']==1){
                $new['area']=$new['business']=$new['client']=0;
            }elseif($data['position']==2){
                $new['business']=$new['client']=0;
                if($new['position']){
                    $new[$job[$new['position']]]=$data['first_leader'];
                }
            ...
            unset($new['position']);
            //如果本人职级发生变化
           if($data['position'] != $user['position']){
               $new['position']=$data['position'];//职位
               if($new[$job[$data['position']]]){
                   $new['original_superior']=$new[$job[$data['position']]];//修改以前的上级为推荐人的上级
               }
               $new[$job[$data['position']]]=0;//将自己级别对应数据清空

           }
           if($new){//职级有发生变化就修改
               $add_mem = $new;
               $row =   M('Users')->where(['user_id'=>$user['user_id']])->update($new);//修改自己的信息
               //修改所有下级的信息
               $conditions['leaders']=array('LIKE',"%.".$uid.".%");
               $list=   M('Users')->where($conditions)->field('user_id,leaders,area,business,client')->select();
               if($list){
                   $new[$job[$data['position']]]=$uid;//将下级与自己级别对应数据添加为职级ID
                   if($new['leaders']){
                       $newleaders=substr($new['leaders'],0,(strlen($new['leaders'])-1));
                   }
                   foreach ($list as $k =>&$v){
                       if($newleaders){
                           $v['leaders'] =$newleaders.$v['leaders'];
                       }
                       if($new['area']){
                           $v['area']=$new['area'];
                           if(($v['area'] == $v['business']) || ($v['area'] == $v['client'])){
                               $v['area']=$new['area'];
                               $v['business']=0;
                               $v['client']=0;
                           }
                       ...
                       if($new['business']){
                           $v['business']=$new['business'];
                           if($v['client'] == $v['business']){
                               $v['client']=0;
                           }
                       }
                       if($new['client']){
                    ...
                    $row = model('Users')->saveAll($list); //修改所有下级的信息
               }
           }
           if($add_mem){//职称有变动 就替换之前的职务
               $data = array_merge($data,$add_mem);
           }
           if($data['level']==5){
               $data['xinyongjin']+=300000;
               $data['is_jihuo']=1;
           }
           $row = M('users')->where(['user_id'=>$data['id']])->update($data);
            if($row)
    ...
    public function rebates(){
        $rebates = M('Rebates')->where('')->find();
        $rebates['proportion']=$rebates['proportion']*100;
        if(request()->isAjax()){
            $data =input();
            if(preg_match('/(^[1-9]{1}[0-9]*$)|(^[0-9]*\.[0-9]{2}$)/',input('proportion'))){
                $data['proportion'] = $data['proportion']/100;
                $bool = M('Rebates')->where(['id'=>$data['id']])->update($data);
                if($bool){
                    $return = ['status' => 1, 'msg' => '修改成功', 'result' => ''];
                }else{
                    $return = ['status' => 0, 'msg' => '修改失败，数据库未响应', 'result' => ''];
                }
            }else {
                $return = ['status' => 0, 'msg' => '修改失败,返点比例不正确!', 'result' => ''];
            }
            $this->ajaxReturn($return);
        }
    ...
    public  function jobTitle(){
        $jobList= M('Manager')->where('')->select();
