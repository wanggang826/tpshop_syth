...
class User extends Base
{
    ...
    public function index()
    {
        ...
        if (empty($user['qrcode'])) {
            ...
            $qrcode_img = createInviteQrcode($url_data, $user_id);
            M('users')->where('user_id', $user_id)->save(array('qrcode' => $qrcode_img));  //存数据库
        }
        $user = $user['result'];
        $level = M('user_level')->select();
        $level = convert_arr_key($level, 'level_id');
        $credit = M('Staging')->where(['user_id' => $user_id, 'status' => 0])->select();
        $user['junior'] = count(junior($this->user_id));//下级人数
        $url = Request::instance()->domain() . '/mobile/user/reg/invite/' . $user_id;  //二维码带推广用户id跳转到注册页面
        $this->assign('credit', count($credit));
...
//        $job = ['1' => 'area', '2' => 'business', '3' => 'client'];// 职级字段
        ...
        $Junior = junior($this->user_id);//下级数组
//        $position = M('Users')->where(['user_id' => $this->user_id])->field('position')->find();//获取职级
//        $where[$job[$position['position']]] = $this->user_id;
        $id= ".".$this->user_id.".";
        $where['leaders'] = array('like', "%$id%");
        $where['page'] = $page;
        $JuniorList = model('Users')->getList($where);
        $list = $JuniorList->render();
        $junior_money = junior_money($this->user_id);//所有下级消费记录
        $points = array_sum(array_map(create_function('$val', 'return $val["pay_points"];'), $junior_money['points']));//积分消费总额
        $money = array_sum(array_map(create_function('$val', 'return $val["user_money"];'), $junior_money['money']));//现金消费总额
        $moneyAll = abs($points + $money);
        $this->assign('Junior', $Junior);//总人数
        $this->assign('JuniorList', $JuniorList);//下级详情
        $this->assign('page', $list);//分页
    ...
    public function consumption()
    {
        ...
        if (is_numeric($user_id)) {
            $list = model('AccountLog')->getList($user_id, $page);
            $page = $list->render();
...
//        $reg = '/^[1-9]+(.[0-9]{2})?$/';
//        if(!preg_match($reg,$number)) return array('status'=>-1,'msg'=>'金额有误!');
//
//
//    }
    /*
    ...
    {
        $where['page'] = I('page') ? I('page') : 1;
        $where['user_id'] = session('user.user_id');
        $list = model('Staging')->getStaging($where);
        $page = $list->render();
        $this->assign('list', $list);
        $this->assign('page', $page);
        return $this->fetch();
    ...
    public function also()
    {
        $id = trim(I('id'));
        $user_id = session('user.user_id');
        $good_info = M('Staging')->where(['user_id' => $user_id, 'id' => $id, 'status' => 0])->find();
        $each_issue = round($good_info['percent_money'] / 100, 2);//每期还款金额
        $yihuan = $each_issue * (36 - $good_info['surplus_nper']); //已还金额
        $money = round(($good_info['percent_money'] - $yihuan), 2);//剩余还款金额;
        if ($money > $good_info['goods_price']) {
            $money = $good_info['goods_price'];
        }
        if ($good_info) {
            $menber = M('Users')->where('user_id', $user_id)->field('user_id,pay_points')->find();
            if ($menber['pay_points'] >= $money) {
                $User = M('Users')->where('user_id', $user_id)->setDec('pay_points', $money);//扣除还款
                if ($User) {
                    accountLog($user_id, '', "-$money", '商品提前还款'); //支付成功添加积分交易记录
                    reimbursement($user_id, array($good_info['id']));//调用奖励机制
                    $good_info['surplus_money'] = 0;//剩余还款金额
                    $good_info['surplus_nper'] = 0;//剩余还款期数
                    $good_info['status'] = 1;//分期状态(已完成)
                    $Staging = M('Staging')->where(['user_id' => $user_id, 'status' => 0, 'id' => $id])->save($good_info, ['id' => $good_info['id']]);//修改分期表
                    if ($Staging) {
                        return json(['status' => -1, 'msg' => '还款成功!']);
                    }
                }
            } else {
                return json(['status' => -1, 'msg' => '还款失败,请充值积分!']);
            }
        }
        return json(['status' => -1, 'msg' => '还款失败!']);
    }
    /**
     * 账户信用金
     */
    public function credit()
    {
        $page = I('page') ? I('page') : 1;
        $user = session('user');
        $xinyong = M('Users')->where(['user_id' => $user['user_id']])->field('xinyongjin,user_id')->find();
        $list = model('Credit')->getCredit(['user_id' => $user['user_id'], 'page' => $page]);//where(['user_id'=>$user['user_id']])->select();

        $page = $list->render();
        $this->assign('page', $page);
        $this->assign('list', $list);
        $this->assign('xinyong', $xinyong['xinyongjin']);
        return $this->fetch();
    }
    ...
    public function credit_recharge()
    {
        $user_id = session('user.user_id');
        if ($user_id) {
            $jifen = M('Users')->where(['user_id' => $user_id])->field('user_id,pay_points,xinyongjin')->find();
            if ($jifen['pay_points'] < 1600) return json(['status' => -1, 'msg' => '积分不够本次换购,请充值积分!']);
            $jifen['pay_points'] -= 1600;
            $jifen['xinyongjin'] += 300000;
            $jifen['level']     =5;
            $data['is_jihuo']=1;
            $bool =M('Users')->where(['user_id'=> $jifen['user_id']])->update($jifen);
            if($bool){
                exchange($user_id);//奖励
                accountLog($jifen['user_id'], '',"-1600", '换购信用金!'); //支付成功添加积分交易记录
                creditLog($jifen['user_id'], '',"+300000", '充值'); //支付成功添加信用金交易记录
                return json(['status'=>1,'msg'=>'积分换购信用金成功!']);
            }else {
                return json(['status' => -1, 'msg' => '积分换购信用金失败!']);
            }
        }
    }
    ...
    public function code()
    {
        ...
        if ($type == 'phone') {//短信验证
            ...
            $mail->Password = 'flmufqaleiwqbhfd';                 //这里填写发件邮箱的POP3/SMTP服务,第三方授权登录密码
            $mail->AddAddress('365105045@qq.com', '强哥');//收件人邮箱,新浪邮箱,QQ邮箱等等都可以
            $mail->IsHTML();
            $mail->Subject = '发送自PHPMailer的邮箱';       //发文件的标题
            $mail->Body = '强哥吉祥!很好,很好,很好啊!';    //发送文件的内容
            $mail->AltBody = '附加信息';                   //发送文件的附加信息
            $mail->From = '365105045@qq.com';             //发送人的地址
            $mail->FromName = '强哥';                     //发送人的姓名
            if ($mail->send()) {
...
//                if($reg_sms_enable){   //是否开启手机注册验证码机制
    ...
    */
    ...
    public function recharge()
    {
        if (IS_POST) {
            ...
            $data['order_sn'] = make_order_sn();
            $data['ctime'] = time();
            $order_id = M('recharge')->add($data);
            if ($order_id) {
                $url = U('Home/Cart/cart4', array('order_id' => $order_id, 'type' => 'recharge'));
                $msg['status'] = 1;
                $msg['url'] = $url;
                return json($msg);
            } else {
     ...
     * ajax 激活用户
     */
    public function ajaxJihuo()
    {
        $user = M('users');
        $pay_points = $user->where('user_id', $this->user_id)->getField('pay_points'); //用户的积分数量
        $need_point = tpCache('basic.jihuo'); // 升级会员所扣除的积分数量(配置表里面读取）
        if ($pay_points < $need_point) {
            $data = ['code' => -1, 'msg' => '积分需要大于' . $need_point . '才可以领取信用金!'];
            $this->ajaxReturn($data);
        }
        $pay_points = $pay_points - $need_point;  //变更积分
        $logic = new UsersLogic();
        $data = $logic->doUpgrade($this->user_id, $pay_points);
     ...
     * ajax领取红包
     */
    public function ajaxGetRedPackage()
    {
        $got_red_package = M('users')->where('user_id', $this->user_id)->getField('got_red_package'); //用户是否已经领取了红包
        if ($got_red_package == 1) {
            $this->ajaxReturn(['code' => -1, 'msg' => '红包已经领取过了']);
        }
        $logic = new UsersLogic();
        $data = $logic->get_red_package($this->user_id);
