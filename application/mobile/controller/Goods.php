...
class Goods extends MobileBase {
    ...
    public function goodsList(){
    	...
    	if($count > 0)
    	{
    		$goods_list = M('goods')->where("goods_id","in", implode(',', $filter_goods_id))->order("$sort $sort_asc")->limit($page->firstRow.','.$page->listRows)->select();//$page->firstRow.','.$page->listRows
    ...
    public function goodsInfo(){
        ...
        $goods['integral']   = floor($goods['shop_price']*$goods['rebates']);
    ...
    #####################二次开发#####################
    //生成商品二维码
    public function ajax_goods_qrcode(){
        $goods_id = I('post.goods_id');
        $user_id = I('post.user_id');
        if(empty($user_id)){  //用户未登录，去登录
            header("location:" . U('Mobile/User/login'));
        }
        $goodsQr = M('user_goods_qrcode')->where(array('goods_id'=>$goods_id, 'user_id'=>$user_id))->getField('qrcode');
        if(empty($goodsQr)){  //如果没有二维码则去生成
            $url = Request::instance()->domain().'/mobile/goods/goodsInfo/id/'.$goods_id.'/invite_user/'.$user_id;  //商品二维码带推广用户id跳转到注册页面
            $path = './public/upload/qrcode/goods_qrcode'.md5($goods_id.time()).'.png';
            $goodsQr = make_qrcode($url, $path);
            //M('user_goods_qrcode')->add(array('user_id'=>$user_id, 'goods_id'=>$goods_id, 'qrcode'=>$goodsQr));  //存数据库
        }
        if($goodsQr != ""){
            $this->ajaxReturn(['code'=>1, 'qrcode'=>$goodsQr]);
        }else{
            $this->ajaxReturn(['code'=>-1, 'goods_qr'=>""]);
        }
