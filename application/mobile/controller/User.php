...
class User extends MobileBase
{
    ...
    */
    ...
    public function index()
    {
        ...
        $credit = M('Staging')->where(['user_id'=>$user_id,'status'=>0])->select();//分期数
        $user['junior'] = count(junior($user_id));//下级人数
        //如果没有二维码，则生成一个
        ...
        $this->assign('level_num',$user['junior']);
        return $this->fetch();
    }
    /**
     * 用户分期详情
    ...
    public function staging(){
        ...
        $where['status']=0;
        $list =  M('Staging')->where($where)->select();//getStaging($where);
    ...
    public function also(){
        ...
        $money= round(($good_info['percent_money']-($yihuan+$good_info['percent_money']*0.64)),2);//剩余还款金额;
        if($money>($good_info['percent_money']*0.36)){
            $money=$good_info['percent_money']*0.36;
        }
        if($good_info){
            $menber = M('Users')->where('user_id',$user_id)->field('user_id,pay_points,xinyongjin')->find();
            if($menber['pay_points'] >= $money){
                $xinyong = round(($money*25/9),2);//应该增加的信用金
                $menber['pay_points'] =$menber['pay_points']-$money;
                $menber['xinyongjin'] =$menber['xinyongjin']+$xinyong;
                $User= M('Users')->where('user_id',$user_id)->update($menber);//扣除还款
                if($User){
     ...
     * 信用金记录
     */
    public function xinyong_list(){
       $user_id = session('user.user_id');
       $type = I('type')?I('type'):'all';
       if($type == 'all'){
           $list = M('Credit')->where(['user_id'=>$user_id])->order('create_time DESC')->select();
           $this->assign('type','all');
       }elseif($type == 'minus'){
           $list = M('Credit')->where(['user_id'=>$user_id,'pay_credit'=>['<',0]])->order('create_time DESC')->select();
           $this->assign('type','minus');
       }
      $this->assign('list',$list);
    ...
    public function memberInfo(){
       $pagennum = I('offset') ? I('offset') : 0;
        $Junior = junior($this->user_id);//下级数组
        $id= ".".$this->user_id.".";
        $where['leaders'] = array('like', "%$id%");

        $JuniorList = model('Users')->getList($where, $pagennum);
        if(request()->isAjax()){
            return json($JuniorList);
        }
...
//        $this->assign('Junior', count($Junior));//总人数
...
//            header("Location: " . U('Mobile/User/index'));
        ...
        $logo = tpCache('shop_info.store_logo');
...
//            }
        ...
        if(!empty($_GET['invite'])){
            $this->assign('invite', $_GET['invite']); //如果接收到了邀请人id则把id放在视图通过表单隐藏域post提交
        }
...
//            header("Location: " . U('User/Index'));
    ...
    /** 前端直接填写充值页面提交到这里
     * @return \think\response\Json 一个请求到cart4的地址
     */
...
//                $this->redirect($url);
                $url = U('Mobile/Cart/cart4',array('order_id'=>$order_id,'type'=>'recharge'));
                $msg['status'] =1;
                $msg['url'] =$url;
    ...
    public function paypwd()
    {
        ...
        $step = I('step', 1);
        if ($step > 1) {
            $check ='585254';// session('validate_code');
            if (empty($check)) {
...
//            }
    ...
    public function upgradeLevel(){
        ...
        if($type == 'jihuo'){
            ...
            $need_point = tpCache('basic.reg_integral'); // 升级会员所扣除的积分数量(配置表里面读取）
        }
        if($pay_points < $need_point){
            $data = ['code'=>-1, 'msg'=>'抱歉，您的积分不足!'];
            $this->ajaxReturn($data);
        }
        $pay_points = $pay_points - $need_point;  //变更积分
        $logic = new UsersLogic();
        $data = $logic->doUpgrade($this->user_id, $type, $pay_points);
        if ($data['status'] != 1)
            $this->ajaxReturn($data['msg']);
        $this->ajaxReturn($data);
    }
    ...
    public function get_qrcode(){
        $qrcode = M('users')->where(array('user_id'=>$this->user_id))->getfield('qrcode');
