 ...
 * $order_id  订单id
 */
function  rebates($order_id){
    $user = M('Order')->where('order_id',$order_id)->field('user_id')->find();
    $user_id = $user['user_id'];
    $order_list = M('OrderGoods')->where(['order_id'=>$order_id,'is_fenqi'=>0])->field('member_goods_price,goods_num,rebates')->select();//查未分期的商品金额
    $money = 0;//订单中未分期付款的商品成交价总和
    foreach ($order_list as $k =>$v){
        $money += $v['member_goods_price']*$v['goods_num']*$v['rebates'];
    }
    $rebates_money = round($money,2);//该用户获得的返点
    M('Users')->where(['user_id'=>$user_id])->setInc('pay_points',$rebates_money);
    accountLog($user_id, '',$rebates_money, '消费返点!','',$order_id ); //支付成功添加积分交易记录
}
 ...
 * requery   bool;
 */
function rewards($member_id,$order_id){
    $member = M('Users')->where(['user_id'=>$member_id])->field(['position','client','business','area'])->find();//获取用户上级
    $order_list = M('OrderGoods')->where(['order_id'=>$order_id,'is_fenqi'=>0])->field('member_goods_price,goods_num')->select();//查未分期的商品金额
    $money = 0;//订单中未分期付款的商品成交价总和
    foreach ($order_list as $k =>$v){
        $money += $v['member_goods_price']*$v['goods_num'];
    }
    //购买者不参与提成,(只奖励上级). 2.基本设置是客户经理5%业务经理0.5%区域经理0.2%如果直推直接拿业务经理拿5.5%区域经理拿5.7%。
    $member_arr=[];//参与提成人员以及提成比例
    if($member['position']){//用户有职称
        if($member['position']==1) {//购买用户为区域经理
            $member_arr[$member_id] =5.7;//区域经理购买,区域经理提成5.7%
        }elseif($member['position']==2){//购买用户为业务经理
            if($member['area']){
                $member_arr[$member['area']] =0.2;//业务经理购买,区域经理提成0.2%
            }
            $member_arr[$member_id] =0.5;//业务经理购买,业务经理提成0.5%
        }elseif($member['position']==3){//购买用户为客户经理
            if($member['business']){//客户经理购买,如果客户有业务经理
                $member_arr[$member['business']] =0.5;//客户经理购买,业务经理提成0.5%
                if($member['area']){//客户经理购买,如果同时有业务经理和区域经理
                    $member_arr[$member['area']] =0.2;//业务经理购买,区域经理提成0.2%
                }
            }else{//如果此客户经理 没有业务经理
                if($member['area']){//客户经理购买,如果没有业务经理,只有区域经理
                    $member_arr[$member['area']] =0.7;//业务经理购买,区域经理提成0.7%
                }
            }
            $member_arr[$member_id] =5;//客户经理购买,客户经理提成5%
        }
    }else{//用户没有职称
        if($member['area'] && $member['business'] && $member['client']){//会员的3个上级都存在
            $member_arr[$member['area']] =0.2;//会员购买,区域经理提成0.2%
            $member_arr[$member['business']] =0.5;//会员购买,业务经理提成0.5%
            $member_arr[$member['client']] =5;//会员购买,客户经理提成5%
        }elseif($member['area'] && $member['business']){//会员只有业务经理 和区域经理
            $member_arr[$member['area']] =0.2;//会员购买,区域经理提成0.2%
            $member_arr[$member['business']] =5.5;//会员购买,业务经理提成5.5%
        }elseif($member['area'] &&  $member['client']){//会员只有客户经理 和区域经理
            $member_arr[$member['area']] =0.7;//会员购买,区域经理提成0.7%
            $member_arr[$member['client']] =5;//会员购买,客户经理提成5%
        }elseif( $member['business'] && $member['client']){//会员只有业务经理 和客户经理
            $member_arr[$member['business']] =0.5;//会员购买,业务经理提成0.5%
            $member_arr[$member['client']] =5;//会员购买,客户经理提成5%
        }elseif($member['client']){//会员只有客户经理
            $member_arr[$member['client']] =5;//会员购买,客户经理提成5%
        }elseif( $member['business']){//会员只有业务经理
            $member_arr[$member['business']] =5.5;//会员购买,业务经理提成5.5%
        }elseif($member['area'] ){//会员只有区域经理
            $member_arr[$member['area']] =5.7;//会员购买,区域经理提成5.7%
        }
    }
    $len = count($member_arr);//提成人数
    foreach ($member_arr as $p =>$q){
        $member_money = $money*$q/100;
        $bool=  M('Users')->where(['user_id'=>$p])->setInc('pay_points',$member_money);//给上级提成
        if($bool){
            accountLog($p, '',$member_money, '下级消费提成','',$order_id );
            $len = $len-1;//成功奖励一个人;提成人数减1,
        }
    }
    if(!$len){//所有人都成功提成
        return true;
 ...
 * 会员换购信用金奖励
 */
function exchange($user_id){
    $member = M('Users')->where(['user_id'=>$user_id])->field(['position','client','business','area'])->find();//获取用户上级
    $member_arr=[];//参与提成人员以及提成比例
    if($member['position']){//用户有职称
        if($member['position']==1){//购买用户为区域经理
            $member_arr[$user_id] =800;//区域经理提成800
        }elseif($member['position']==2){//购买用户为业务经理
            if($member['area']){
                $member_arr[$member['area']] =20;//业务经理购买,区域经理提成20
            }
            $member_arr[$user_id] =780;//业务提成780
        }elseif($member['position']==3){//购买用户为客户经理
            if($member['business']){//客户经理购买,如果客户有业务经理
                $member_arr[$member['business']] =100;//客户经理购买,业务经理提成100
                if($member['area']){//客户经理购买,如果同时有业务经理和区域经理
                    $member_arr[$member['area']] =20;//业务经理购买,区域经理提成0.2%
                }
            }else{//如果此客户经理 没有业务经理
                if($member['area']){//客户经理购买,如果没有业务经理,只有区域经理
                    $member_arr[$member['area']] =120;//业务经理购买,区域经理提成0.7%
                }
            }
            $member_arr[$user_id] =680;//客户经理提成680
        }
    }else{//用户没有职称
        if($member['area'] && $member['business'] && $member['client']){//会员的3个上级都存在
            $member_arr[$member['area']] =20;//会员购买,区域经理提成0.2%
            $member_arr[$member['business']] =100;//会员购买,业务经理提成0.5%
            $member_arr[$member['client']] =680;//会员购买,客户经理提成5%
        }elseif($member['area'] && $member['business']){//会员只有业务经理 和区域经理
            $member_arr[$member['area']] =20;//会员购买,区域经理提成0.2%
            $member_arr[$member['business']] =780;//会员购买,业务经理提成5.5%
        }elseif($member['area'] &&  $member['client']){//会员只有客户经理 和区域经理
            $member_arr[$member['area']] =120;//会员购买,区域经理提成0.7%
            $member_arr[$member['client']] =680;//会员购买,客户经理提成5%
        }elseif( $member['business'] && $member['client']){//会员只有业务经理 和客户经理
            $member_arr[$member['business']] =100;//会员购买,业务经理提成0.5%
            $member_arr[$member['client']] =680;//会员购买,客户经理提成5%
            ...
            $member_arr[$member['area']] =800;//会员购买,区域经理提成5.7%
        }
    }
    $len = count($member_arr);//提成人数
    foreach ($member_arr as $p =>$q){
        $bool=  M('Users')->where(['user_id'=>$p])->setInc('pay_points',$q);//给上级提成
        if($bool){
            $len = $len-1;//成功奖励一个人;提成人数减1,
            accountLog($p, '',$q, '换购信用金提成');
        }
    }
    if(!$len){//全部都成功提成,
        return true;
 ...
 * 会员自主提前还款奖励机制
 * $member_id 购买者ID
 * $order_goods_id  arr() 提前还款的商品(sh_Staging表里面的id字段)
 * requery   bool;
 */
function reimbursement($member_id,$order_goods_id=array()){
    $member = M('Users')->where(['user_id'=>$member_id])->field(['position','client','business','area'])->find();//获取用户上级
    $member_arr=[];//参与提成人员以及提成比例
    $order_goods = M('Staging')->where(['user_id'=>$member_id,'id'=>['in',$order_goods_id]])->field('surplus_money,percent_money')->select();//用户还款的商品目前的欠款
    $money= array_sum(array_map(create_function('$val', 'return $val["surplus_money"]-$val["surplus_money"]*0.64;'), $order_goods));//用户总的还款金额
    //会员还款成功之后客户经理奖励总扣款积分客户经理奖励的15%，业务经理奖励3%，区域经理0.5%。
    if($member['position']){//用户有职称
        if($member['position']==1){//还款用户为区域经理
            $member_arr[$member_id] =18.5;//区域经理还款,区域经理提成18.5%
        }elseif($member['position']==2){//还款用户为业务经理
            if($member['area']){
                $member_arr[$member['area']] =0.5;//业务经理还款,区域经理提成0.5%
            }
            $member_arr[$member_id] =3;//业务经理还款业务经理提成3%
        }elseif($member['position']==3){//还款用户为客户经理
            if($member['business']){//客户经理还款,如果客户有业务经理
                $member_arr[$member['business']] =3;//客户经理还款,业务经理提成3%
                if($member['area']){//客户经理还款,如果同时有业务经理和区域经理
                    $member_arr[$member['area']] =0.5;//业务经理购买,区域经理提成0.5%
                }
            }else{//如果此客户经理 没有业务经理
                if($member['area']){//客户经理还款,如果没有业务经理,只有区域经理
                    $member_arr[$member['area']] =3.5;//业务经理还款,区域经理提成3.5%
                }
            }
            $member_arr[$member_id] =15;//客户经理还款;客户经理提成15%
        }
    }else{//用户满意职称
        if($member['area'] && $member['business'] && $member['client']){//会员的3个上级都存在
            $member_arr[$member['area']] =0.5;//会员还款,区域经理提成0.5%
            $member_arr[$member['business']] =3;//会员还款,业务经理提成3%
            $member_arr[$member['client']] =15;//会员还款,客户经理提成15%
        }elseif($member['area'] && $member['business']){//会员只有业务经理 和区域经理
            $member_arr[$member['area']] =0.5;//会员还款,区域经理提成0.5%
            $member_arr[$member['business']] =18;//会员还款,业务经理提成18%
        }elseif($member['area'] &&  $member['client']){//会员只有客户经理 和区域经理
            $member_arr[$member['area']] =3.5;//会员还款,区域经理提成3.5%
            $member_arr[$member['client']] =15;//会员还款,客户经理提成15%
        }elseif( $member['business'] && $member['client']){//会员只有业务经理 和客户经理
            $member_arr[$member['business']] =3;//会员还款,业务经理提成3%
            $member_arr[$member['client']] =15;//会员还款,客户经理提成15%
            ...
            $member_arr[$member['area']] =18.5;//会员还款,区域经理提成18.5%
        }
    }
    $len = count($member_arr);//提成人数
    foreach ($member_arr as $p =>$q){
        ...
        if($bool){
            $len = $len-1;//成功奖励一个人;提成人数减1,
            accountLog($p, '',$member_money, '下级提前还款提成');
        }
    }
    if(!$len){//全部都成功提成,
        return true;
...
function manager()
{
    $list = model('Users')->where(['position' => ['in', [1, 2, 3]]])->field('user_id,nickname,position')->select();//所有经理
    $manager = [];
    foreach ($list as $k => $v) {
        if ($v['position'] == 1) {
            $manager['area'][] = $list[$k];
        } elseif ($v['position'] == 2) {
            $manager['business'][] = $list[$k];
        } elseif ($v['position'] == 3) {
            $manager['client'][] = $list[$k];
        }
    }
    return $manager;
}
 ...
 * 所有下级()
 */
function junior($user_id)
{
    $id= ".".$user_id.".";
    $where['leaders'] = array('like', "%$id%");
    $position = M('Users')->where(['user_id' => $user_id])->field('position')->find();//获取职级
    if ($position['position']) {
        return M('Users')->where($where)->field('user_id,nickname,mobile,pay_points,level,last_login,position')->select();
    } else {
        return array();
    }
 ...
 * 获取所有下级的消费
 */
function junior_money($user_id)
{
    $job = ['1' => 'area', '2' => 'business', '3' => 'client'];// 职级字段
    $position = M('Users')->where(['user_id' => $user_id])->field('position')->find();//获取职级
    $idAll = M('Users')->where([$job[$position['position']] => $user_id])->column('user_id');//所有下级ID
    $list['points'] = M('AccountLog')->where(['user_id' => ['in', $idAll], 'pay_points' => ['<', 0]])->select();//所有下级积分消费
    $list['money'] = M('AccountLog')->where(['user_id' => ['in', $idAll], 'user_money' => ['<', 0]])->select();//所有下级现金消费
    return $list;
}
...
function goods_thum_images($goods_id, $width, $height)
{
    ...
    if (empty($original_img)) {
        ...
        return tpCache('shop_info.store_logo');
    }
...
function send_email($to, $subject = '', $content = '')
{
    ...
    $mail->Username = 'wangxiao_6349@qq.com';  //这里填写发件邮箱账号
    $mail->Password = 'snfmnoemtsakbiaf';      //这里填写发件邮箱的POP3/SMTP服务,第三方授权登录密码
    $mail->AddAddress($to);//收件人邮箱,新浪邮箱,QQ邮箱等等都可以
    $mail->IsHTML();
    $mail->Subject = $subject;       //发文件的标题
    $mail->Body = '验证码:' . $content . '.如非本人操作,请忽略!';    //发送文件的内容
    $mail->AltBody = '附加信息';                   //发送文件的附加信息
    $mail->From = 'wangxiao_6349@qq.com';             //发送人的地址
    $mail->FromName = '顺亿商城';                     //发送人的姓名
    if (!$mail->send()) {
        $msg = [
            'status' => -1,
            'msg' => '发送失败!'
        ];
        return $msg;//array('status'=>-1 , 'msg'=>'发送失败!');
    } else {
        $msg = [
            'status' => 1,
            'msg' => '发送成功!'
        ];
        return $msg;//array('status'=>1 , 'msg'=>'发送成功');
    }
}
...
function update_pay_status($order_sn, $ext = array())
{
    ...
    if (stripos($order_sn, 'recharge') !== false) {
        ...
        Db::startTrans();
        try {
            //充值订单支付状态修改 //积分增加 //添加日志
            M('recharge')->where(array("order_sn" => $order_sn))->save(array('pay_status' => 1, 'pay_time' => time()));
            M('users')->where(array("user_id" => $order['user_id']))->setInc('pay_points', $order['account']);
            accountLog($order['user_id'], -$order['account'], $order['account'], '会员在线充值','','',$order_sn);
            Db::commit();
        } catch (\Exception $e) {
            Db::rollback();
        }
        ...
        // 修改支付状态  已支付
        $member = M('Users')->where(['user_id' => $order['user_id']])->find();
        if ($order['xinyong']>0) { //查看有没有使用信用金,有使用就要扣去用户信用金
            $xinyong = $order['xinyong'];//支付的信用金
            $member['xinyongjin'] = $member['xinyongjin'] - $xinyong;
            $member['months_xinyong'] = $member['months_xinyong'] - $xinyong;
            M('Users')->update($member);
            creditLog($order['user_id'], $order['order_id'], "-$xinyong", '分期抵扣!'); //支付成功添加信用金交易记录
        }
        ...
        staging_edit($order['order_id']);//修改分期表
        rewards($order['user_id'], $order['order_id']); //经理奖励
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '')
{
    ...
    $xinyong = 0;
    $integral = 0;
    $goods_price = $cut_fee = $anum = $goods_weight = 0;
    foreach ($order_goods as $key => $val) {
        ...
        if ($order_goods[$key]['is_fenqi']) {
            $is_fenqi = $order_goods[$key]['is_fenqi'];
//            $xinyong  += round(($order_goods[$key]['member_goods_price']*(1-$order_goods[$key]['percent'])),2);//信用金支付部分
//            $integral += round($order_goods[$key]['member_goods_price']-round(($order_goods[$key]['member_goods_price']*(1-$order_goods[$key]['percent'])),2),2);//积分支付部分
            $xinyong += round(($order_goods[$key]['goods_fee'] * (1 - $order_goods[$key]['percent'])), 2);//信用金支付部分
            $integral += round(($order_goods[$key]['goods_fee'] * ($order_goods[$key]['percent'])), 2);//信用金支付部分
        } else {
            $integral += $order_goods[$key]['goods_fee'];
        }
    }
    // 优惠券处理操作
    ...
    $order_amount = $goods_price + $shipping_price; // 应付金额 = 商品价格 + 物流费 - 优惠券
    ...
    $result = array(
        ...
        'is_fenqi' => $is_fenqi,//是否分期
        'xinyong' => $xinyong,//分期金额
        'integral' => $integral,//使用积分金额
    );
