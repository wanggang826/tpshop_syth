...
class OrderLogic
{
	 ...
	 * @return array
	 */
	public function addOrder($fenqi,$xinyong,$user_id,$address_id,$shipping_code,$invoice_title,$coupon_id = 0,$car_price,$user_note='',$pay_name='')
	{
		...
		$data = array(
				...
				'integral'         =>$car_price['integral'], //'应付积分（和order_amount字段值相同）',
				'integral_money'   =>$car_price['pointsFee'],//'使用积分抵多少钱',
				'order_amount'     =>$car_price['integral'],//'应付金额',
                'total_amount'     =>$car_price['payables'],//'总金额',
				'add_time'         =>time(), // 下单时间
		...
		foreach($cartList as $key => $val)
		{
            ...
            $data2['is_fenqi']           = $val['is_fenqi']; // 是否分期
            $data2['percent']            = $val['is_fenqi']?$val['percent']:0; // 分期比例
            $data2['percent_money']      = $val['is_fenqi']?round(($val['member_goods_price']*$goods['percent']),2):0;//分期金额(贷款金额)
            $data2['surplus_money']      = $val['is_fenqi']?round(($val['member_goods_price']*$goods['percent']),2):0;;//分期剩余金额(贷款未还部分的金额)
			$data2['rebates']            = $val['rebates'];//返点比例
        ...
        {
            if($v['is_fenqi']){
                $goodsss = M('goods')->where("goods_id", $val['goods_id'])->cache(true,TPSHOP_CACHE_TIME)->find();
                $staging['user_id']           = $user_id; // 订单id
                $staging['order_id']           = $order_id; // 订单id
                $staging['goods_id']           = $v['goods_id']; // 商品id
                $staging['goods_name']         = $v['goods_name']; // 商品名称
                $staging['goods_sn']           = $v['goods_sn']; // 商品货号
                $staging['goods_num']          = $v['goods_num']; // 购买数量
                $staging['goods_price']        = $v['goods_price']; // 商品价               为照顾新手开发者们能看懂代码，此处每个字段加于详细注释
                $staging['spec_key']           = $v['spec_key']; // 商品规格
                $staging['spec_key_name']      = $v['spec_key_name']; // 商品规格名称
                $staging['member_goods_price'] = $v['member_goods_price']; // 会员折扣价
                $staging['prom_type']          = $v['prom_type']; // 0 普通订单,1 限时抢购, 2 团购 , 3 促销优惠
                $staging['prom_id']            = $v['prom_id']; // 活动id
                $staging['percent']            = $v['percent']; // 分期比例
                $staging['percent_money']      = round(($v['member_goods_price']*(1-$v['percent'])*$v['goods_num']),2);//分期金额(贷款金额)
                $staging['surplus_money']      = round(($v['member_goods_price']*(1-$v['percent'])*$v['goods_num']),2);//分期剩余金额(贷款未还部分的金额)
                $staging['create_time']         = time();
                $order_goods_id              = M("Staging")->insertGetId($staging);
            }
