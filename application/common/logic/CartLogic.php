...
class CartLogic extends Model
{
    ...
    protected $is_fenqi = 0;//是否分期
    protected $percent = 0;//分期比例
     ...
     * @param $goods_id
     */
    public function setIsFenqi($is_feiqi)
    {
        $this->is_fenqi = $is_feiqi;
    }
    ...
    public function addGoodsToCart()
    {
        ...
        $this->percent = $this->goods['percent'];
        //有商品规格，和没有商品规格
    ...
    private function addNormalCart(){
        ...
        if (!$this->user_id) {
            ...
            $userCartGoods = Cart::get(['is_fenqi'=>$this->is_fenqi,'user_id'=>$this->user_id,'goods_id'=>$this->goods['goods_id'],'spec_key'=>($this->specGoodsPrice['key'] ?: '')]);
        }
        // 如果该商品已经存在购物车
        if ($userCartGoods) {
            ...
            $cartAddData = array(
                ...
                'prom_id' => 0,   // 活动id
                'is_fenqi'=>$this->is_fenqi,
                'percent' =>$this->percent,
                'rebates' =>$this->goods['rebates'],   // 返点比例
            );
            if($this->specGoodsPrice){
    ...
    public function getCartList($selected = 0){
        ...
        $cartList = $cart->where($cartWhere)->select();  // 获取购物车
...
//            dump($cart['goods_fee']);
            //是否分期
            if($cart['is_fenqi'] ==2) {
                $cart['goods_fee'] = round($cart['goods_fee'] * $cart['goods']['percent'], 2);
                $min_num = M('Goods')->where(['goods_id'=>$cart['goods_id']])->field('fenqi_num')->find();
                $cart['min_num']=$min_num['fenqi_num'];
            }
//            dump($cart['goods_fee']);
            //商品不存在或者已经下架
...
//        dump($cartList);die;
    ...
    /**
     * 获取购物车的价格详情(仅限购物车页面使用)
     * @param $cartList|购物车列表
     * @return array
     */
    public function getCartPriceInfo($cartList = null){
        $total_fee = $goods_fee = $goods_num = $integral = $xinyong = 0;//初始化数据。商品总额/节约金额/商品总共数量/使用积分数/使用信用金数
        if($cartList){
            foreach ($cartList as $cartKey => $cartItem) {
                $total_fee += $cartItem['goods_fee'];
                $goods_fee += $cartItem['cut_fee'];
                $goods_num += $cartItem['goods_num'];
                if($cartItem['is_fenqi']==2) {
                    $integral += round(($cartItem['goods_fee'] * $cartItem['goods']['percent']), 2);
                    $xinyong  += round(($cartItem['goods_fee']-$integral),2);
                }else{
                    $integral +=  $cartItem['goods_fee'];
                }
                $freight_free = tpCache('shopping.freight_free'); // 全场满多少免运费
            }
        }
        return compact('total_fee', 'goods_fee', 'goods_num','integral','xinyong');
    }

    /**
     * 获取购物车的价格详情(仅限提交表单页面使用)
     * @param $cartList|购物车列表
     * @return array
     */
    public function getOrderPriceInf($cartList = null){
        $total_fee = $goods_fee = $goods_num = 0;//初始化数据。商品总额/节约金额/商品总共数量
        if($cartList){
            foreach ($cartList as $cartKey => $cartItem) {

                if($cartItem['is_fenqi']==2){
                    $total_fee += round(($cartItem['goods_fee']*$cartItem['goods']['percent']),2);
                }else{
                $total_fee += $cartItem['goods_fee'];
                }
                $goods_fee += $cartItem['cut_fee'];
