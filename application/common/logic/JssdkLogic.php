...
// 随机字符串
  private function createNonceStr($length = 16) {
    ...
    public function send_template_message($order){
    	$user = session('user');
    	$goods = M('order_goods')->where(array('order_id'=>$order['order_id']))->find();
    	$access_token = $this->get_access_token();
    	$url = "https://api.weixin.qq.com/cgi-bin/message/template/send?access_token={$access_token}";
    	$tempalte = array(
    		'touser'=>$user['openid'],
    		'template_id'=>'nSFQj7UtetEnKM5eeAzdlu_jmizaE_6ds4itHoygnTE',
    		'url' => SITE_URL.'/mobile', //点击后跳转地址
    		'topcolor'=>'#7B68EE',
    		'data' => array('first'=>array('value'=>urlencode("您好，您的订单已支付成功！"),'color'=>'#743A3A')),
    			'product'=>array('value'=>$goods['goods_name'],'color'=>'#FF0000'),
    			'prcie' => array('value'=>$goods['goods_price'],'color'=>'#FF0000'),
    			'time' => array('value'=>date('Y-m-d H:i'),'color'=>'#0000FF'),
    			'remark' => array('value'=>urlencode("\\您的订单已提交，我们将尽快发货。祝您生活愉快！"),'color'=>'#008000')			
    	);
    	$res = httpRequest($url,'post',$tempalte);
